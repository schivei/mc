...
1. **Memory model.** M22 defaults to reference counting with free lists and documented cycle leaks;
   nothing here depends on it (an arena-per-scope or ownership model uses the same mechanisms).
   Confirm rc, or name the alternative before `examples/lang` is written.
2. **Precedence table exposure.** `syntax_infix` shares `#infix`'s table, which is not inspectable.
   Should `--dump-rules` list operator, precedence, associativity and whether a handler is attached?
3. **Colliding registrations.** Two modules registering `syntax_infix` on the same token collide
   silently (one entry, last wins), unlike `syntax`/`syntax_stmt`, whose back-to-front search keeps
   the earlier entry. Refuse the overwrite, or document last-wins?
4. **Instantiation budget.** Each instantiation burns a token-table entry (`MAXTOK 512`), an alias and
   a `syntax_stmt` slot. 2.8 raises two of the three; raise `MAXTOK` now, or measure with M22 first?
5. **Split delivery.** Land 2.1-2.2 (parser hooks) and 2.3-2.7 (record/replay) as two commits, each
   gated by the byte-identity check of 6(5), with the golden rewritten once per commit?
