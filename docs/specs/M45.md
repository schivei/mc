...
The defect, as the M42 agent measured it (`docs/specs/M42.md` § Implementation notes 10, branch `m42-elf-exe`): C's `open` returns `int`; AAPCS64 leaves bits 63..32 of `x0` unspecified for a 32-bit result; glibc on aarch64 hands back `0x00000000ffffffff` for -1; `mc` declares `extern i64 open(...)` (`lib/sys.mc:16`, `src/arena.mc:5`) and reads all of `x0`, so `i64 fd = open(...); if (fd < 0)` is never taken. The same holds on x86-64 (`mov eax, -1` zeroes the upper half of `rax`) and on both Windows ABIs (`BOOL`/`DWORD` are 32-bit). Every C function returning `int`/`unsigned`/`short`/`char` on every target is trusted as a 64-bit value today.
