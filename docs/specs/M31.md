...
1. **Order of work.** 2.3 is nearly free and protects everything; 2.1 is ~20 lines and unblocks
   widening for core-declared callees; 2.2 is ~35 lines and the only one that changes `parse_stmt`'s
   shape. Land all three before the example, or ship the example against `unit_head` plus a `lock`
   that refuses `return`, and land 2.2 after?
2. **Host.** A module *added to* `examples/lang` (needs 2.2 and the chained `{`), or a sibling example
   with its own smaller language (cheaper, proves less)?
3. **Reserved words.** `spawn`, `intent`, `await`, `lock`, `chan` stop being identifiers program-wide;
   `intent` in particular is a plausible variable name. Accept, or prefix?
4. **LSE.** Ship arm64-with-LSE only and refuse otherwise, or wait for M24's `#machine` so a
   pre-ARMv8.1 target gets a correct `ldaxr`/`stlxr` sequence?
5. **Allocator.** Global mutex now with per-thread arenas as a later milestone, or make the reserved
   register (2.4) a prerequisite and do it once?
