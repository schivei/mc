...
1. **`DE_LEN` (SS3.16), in or out of M28?** 6 lines, unrelated to the LSP, verified byte-identical,
   worth 37% of parse time. Recommendation: a separate commit before M28, so its golden rewrite is not
   entangled with the server's.
2. **Does `gen_lower` run on every keystroke?** +66% on top of the parse, and it buys the `unknown name`
   diagnostics plus the authoritative use links. Recommendation: yes, with `[lsp] resolve = "parse"` in
   `mc.toml` as the escape for very large units.
3. **A document outside `[project].entry`'s closure.** Recommendation: scan it as its own root and say
   so; a `[lsp] unit = "file"` key for projects where per-file navigation is enough.
4. **The `kind` enum of `p_decl`** is a vocabulary shared by the core and every module. Adding one later
   is append-only (like `D_DYLIB`/`D_EMBED`), but the initial set decides whether modules end up
   encoding meaning in strings. Recommendation: start from LSP `SymbolKind`, in `docs/reference`.
5. **`fork` on Windows (M19/M20).** The `--lsp-index` spawn fallback is designed in and measured. Accept
   the asymmetry and document it?
6. **Diagnostics beyond the first.** If one squiggle at a time proves unbearable, the cheapest honest
   escalation needs **no core change**: after publishing error N, blank the offending line in the
   overlay and rescan, bounded to 3 iterations (~12 ms). Build it now, or wait for the complaint?
