...
|---|---|---|---|---|
| G1 | `[target]` resolved before `user_init` | defer the resolution | ~25 | yes, for M39 |
...
**G1.** `drv_run` validates and resolves `[target]` at `src/driver.mc:516-518`, while
...
* **`mc build` driving a bare target** (G1). Decided in D2.
...
| `docs/guide/95-a-new-architecture.md` | ~180 | "I have an ISA" -> "my image boots", in three registrations |
...
every new target; (c) `docs/guide/95-a-new-architecture.md` is written for the MIPS developer of
