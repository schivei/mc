...
Owner's request (2026-09-04, verbatim): "um gerenciador de pacotes, para o mc seria no estilo do
go, fornecendo o fonte, o gerenciador seria o repo de fontes registradas." Read as three
...
**The tree hash is Go's `h1` (`golang.org/x/mod/sumdb/dirhash.Hash1`), in hex and in manifest
order:** for `mc.toml` first and then each entry of `[package].files` in the order written, one
...
last. **Nothing is unpacked before the tree is hashed and compared**: the archive itself is not
checksummed (§ 3 says why), so the refusal moves one step later than M25's but is still before
any manifest exists and before any build can consume the tree.
...
The owner's two rulings, verbatim:

> (1) "44. Eu mudaria, como se trata de libs externas e que estarao referenciadas no toml, diria que
> podem ser acessadas como `#include <pack/lib.mc>`, o que isso muda, ate o que servimos como hoje
> embedded poderiam virar um desempacotamento dentro de um diretorio `~/.mc/libs/pack_name/v<version>/`;
> e com isso ganhar ate mais extensibilidade."
>
> (2) "Diria que ganhamos ate a reducao do tamanho do nosso binario, que poderia carregar somente o
> executavel que exige um `mc install` e nisso fazer o download ate do que hoje e embedded, um
> `mc update` atualiza a versao dos pacotes, um `mc upgrade` faz auto atualizacao da versao bem como
> o download das libs basicas para o mc, e por ai vai."
