...
* **program profile** (default, what a `lib/sys_linux.mc` or M42 program gets): `read write openat close fchmod exit_group exit mmap munmap brk mprotect set_tid_address`, plus the dynamic loader's `newfstatat pread64 readlinkat` when the binary has a `PT_INTERP`; glibc's `ld.so` additionally needs `access prlimit64 getrandom rseq set_robust_list arch_prctl` (x86_64) -- a `--libc=gnu` variant of the same table.
