...
| **M12** | Tier 3 só no `.mc`: syntax hooks (`syntax`/`syntax_stmt` + API pública do parser), `type_alias`, `#dylib`; `examples/api` (HTTP + SQLite + `class`/`interface` ensinados por `oop.mc`) | `make -C examples/api test` verde com binário `--exe`; `make check` com `check-examples` | API do parser insuficiente; bind por ordinal; sockets |
| **M13** | (backlog) dimensionar a arena do programa em tempo de compilação — perfil, anotação ou análise de limites (`docs/specs/M13.md`) | `--mem-report` + `HEAP_SIZE` gravado pelo compilador | indecidível no geral; só perfil é universal |
