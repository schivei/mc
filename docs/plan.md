...
| **M25** | Sysroots and cross-compilation resolution: explicit path -> running system -> `~/.mc/sysroots` cache -> precise instructions; Apple text stubs synthesized from the program's externs (no SDK files redistributed); `mc sysroot fetch` for musl (Alpine apk) and mingw-w64 import libs (llvm-mingw) with checksums and offline fallback; `mc sysroot list` (`docs/specs/M25.md`) | link on macOS without an SDK via synthesized stubs; fetch into a temp cache and build for linux; offline paths exit with instructions |
