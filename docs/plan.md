...
| **M33** | WebAssembly (owner, 2026-09-03, last in the queue): a `.mc` backend consuming the AST (structured control flow, `call_indirect` for `callp`, imports for `extern`, data segments, wasm64/memory64 so `uptr` stays 8 bytes) with two writers, binary `.wasm` and text `.wat`; `<sys/wasi>` and `<sys/browser>`; `examples/wasm` with a WASI CLI (run under wasmtime) and a browser page with JS glue (checked in the in-app browser); architecture sweep first to list what the core would need (`docs/specs/M33.md`) | suite subset runs under wasmtime; the page runs in the browser; `.wat` round-trips through `wat2wasm` when installed |
...
| **M35** | Benchmark tooling: `make bench` (compile-time, run-time micro-benchmarks in mc, sizes, RSS) with CSV history and a CI job; numbers page on the site (`docs/specs/M35.md`) | CSV rows deterministic except timings; site page renders |
