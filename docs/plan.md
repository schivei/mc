...
| **M23** | Self-adapting limits: growable tables in the self-hosted `mc`, `[limits]` in `mc.toml` (incl. `heap`), `mc limits` checker (ok/near/over, CI exit codes), `mc build --fix-limits` with the developer's consent, `check-limits` on the seed's headroom (`docs/specs/M23.md`) | 5k-function/5k-string program builds with TOML caps; `--fix-limits` edits only `[limits]`; `make check` warns before the seed's tables fill |
