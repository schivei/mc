...
for t in tests/[0-9]*.lx; do
    ...
    if [ -n "$experr" ]; then
        got=$("$mcconc" --exe "$t" -o "$tmp/$name" 2>&1 >/dev/null)
        if [ "$got" = "$experr" ]; then
            echo "  ok    $name (error)"
            echo "        $got"
        else
    ...
    [ -n "$expexit" ] || expexit=0
    rm -f "$tmp/$name"
    if ! err=$("$mcconc" --exe "$t" -o "$tmp/$name" 2>&1); then
...
"$mcconc" tests/01-await-chain.lx -o "$tmp/a.o" || { echo "FAIL: object 1"; exit 1; }
"$mcconc" tests/01-await-chain.lx -o "$tmp/b.o" || { echo "FAIL: object 2"; exit 1; }
...
mkdir -p "$tmp/x" "$tmp/y"
"$mcconc" --exe tests/01-await-chain.lx -o "$tmp/x/prog" || { echo "FAIL: exe 1"; exit 1; }
"$mcconc" --exe tests/01-await-chain.lx -o "$tmp/y/prog" || { echo "FAIL: exe 2"; exit 1; }
...
"$mcconc" --dump-asm tests/01-await-chain.lx > "$tmp/a.s" || { echo "FAIL: asm 1"; exit 1; }
"$mcconc" --dump-asm tests/01-await-chain.lx > "$tmp/b.s" || { echo "FAIL: asm 2"; exit 1; }
...
got=$("$mc" tests/01-await-chain.lx -o "$tmp/none.o" 2>&1 >/dev/null)
...
if "$mc" lib/conc_rt.mc -o "$tmp/rt.o" 2>/dev/null; then
...
for w in spawn await intent chan lock; do
    printf '#include "%s/lib/prelude.lx"\nfn main() -> i64 { i64 %s = 1; return %s; }\n' \
        "$dir" "$w" "$w" > "$tmp/w/r.lx"
    got=$("$mcconc" --exe "$tmp/w/r.lx" -o "$tmp/w/r" 2>&1 >/dev/null)
